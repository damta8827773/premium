...
```
premium/
├── admin/                 # Admin panel (products, stock, orders, users, vouchers…)
...
├── includes/              # Shared partials (head, sidebars, footer)
...
2. **Create your configuration files** (these hold secrets and are git-ignored)
   ...
   cp assets/js/firebase-init.example.js assets/js/firebase-init.js
   ```

3. **Fill in your credentials**
   - `config/app.php` — Midtrans server/client keys, admin WhatsApp numbers, app URL
   - `assets/js/firebase-init.js` — your Firebase web config

4. **Configure the Midtrans webhook**
   In the Midtrans Dashboard → *Settings → Configuration*, set the
   **Payment Notification URL** to:
   ```
   https://yourdomain.com/api/midtrans-notify.php
   ```
...
|-----|------|-------------|
| `MIDTRANS_SERVER_KEY` | `config/app.php` | **Secret.** Server-side Midtrans key used to sign requests. |
| `MIDTRANS_CLIENT_KEY` | `config/app.php` | Public client key used by Snap on the frontend. |
| `MIDTRANS_IS_PRODUCTION` | `config/app.php` | `true` for live, `false` for sandbox. |
| `ADMIN_WA_1` / `ADMIN_WA_2` | `config/app.php` | Admin WhatsApp contact numbers. |
| `firebaseConfig` | `assets/js/firebase-init.js` | Firebase web app configuration. |
...
`POST /api/midtrans-notify.php` — called by Midtrans. The signature is verified
server-side (`sha512` of `order_id + status_code + gross_amount + server_key`)
before any payment is accepted.

### Check payment status
`GET /api/check-payment.php?order_id=DEP-1700000000`

## Security

- 🔒 Real credentials live only in `config/app.php` and `assets/js/firebase-init.js`, both **git-ignored** — never commit them.
- 🧾 Midtrans webhook requests are verified with a SHA-512 signature before processing.
- 🚫 `.htaccess` blocks direct access to `config/`, `temp/`, and dotfiles, and prevents PHP execution inside `uploads/`.
